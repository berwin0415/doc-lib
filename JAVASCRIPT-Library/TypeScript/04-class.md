...
这个例子演示了**如何在子类里可以重写父类的方法**。`Snake`类和 `Horse` 类都创建了 `move` 方法，它们重写了从 `Animal` 继承来的 `move` 方法，使得 `move` 方法根据不同的类而具有不同的功能。注意，即使 `tom` 被声明为 `Animal` 类型，但因为它的值是 `Horse`，调用 `tom.move(34)` 时，它会调用 `Horse` 里重写的方法。
...
属性和方法默认为public,也可以显式的设置为public
...
`employee` 来源于`Employee`,`animal`来源于`Animal`,虽然都是string类型的name，但不兼容
